...
def add_new_person():
	...
	num = int(input('Enter number of shots: '))
	if not os.path.isdir('Trainingdata/' + name):
		os.mkdir('Trainingdata/' + name)
...
def load_data():
	folders = [name for name in os.listdir('./Trainingdata') if os.path.isdir(name)]
	count =0
	...
	# for name in folders:
		...
		# count +=
	for i,j,k in os.walk('./Trainingdata',topdown=False):
			if not os.path.isdir(i[2:]):
				break
			print('loading data for -->'+ i[2:])
			count+=1
			dic_folders[count] = i[15:]
			# print(i[2:])
			# print(k)
			for name in k:
